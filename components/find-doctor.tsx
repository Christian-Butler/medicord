...
export default function FindDoctor() {
  ...
  return (
    <View className="px-6 pt-6">
      ...
      {loading ? <Text className="mt-3 text-[#7B8A91]">Searching...</Text> : null}
      {error ? <Text className="mt-3 text-[#B10111]">{error}</Text> : null}

      {!loading && !error && searchQuery.trim() && doctors.length === 0 ? (
        <Text className="rounded-b-xl mx-2 px-4 py-2 bg-[white] text-[#7B8A91]">No doctors found.</Text>
      ) : null}
      ...
      {doctors.map((doctor) => (
        ...
        >
          ...
          <View>
            ...
            <Text className="text-[#7B8A91]">{doctor.specialty ?? "Doctor"}</Text>
          </View>
        </Pressable>
      ))}
    </View>
